...
function writeLog(logEntries) {
    try {
        ...
        // 使用日期创建文件路径
        let logExcelPath = `C:/log/237-output_log_${year}${month}${day}.xlsx`;
        let existingData;
        try {
            // 尝试读取现有的 Excel 文件
...
function writeDetailExcel(dataList) {
    try {
        ...
        for (let j = 0; j < dataListLength; j++) {
            let arr = [];
            arr.push("237-Server"); // 新增服务器列，替换为实际的服务器名称
...
function writeCountExcel(dataList) {
    try {
        ...
        // 本地 Excel 路径
        let excelPath = "D:/237Data/count/" + `${year}_${month}_${day}_` + "All_count.xlsx";
        ...
        for (let j = 0; j < dataListLength; j++) {
            ...
            // 添加新的数据
            let arr = [];
            arr.push('237'); // 服务器列，固定为 "237Server"
            arr.push(dataList[j][0]); // 优化时间
            arr.push(dataList[j][1]); // 关键词
            arr.push(dataList[j][2]); // 标题
        ...
        if (localRet === true) {
            addLogEntry('info', deviceName, '成功写入237总count本地Excel文件');
        } else {
...
function writeRoundCountExcel(dataList) {
    try {
        ...
        if (localRet === true) {
            addLogEntry('info',deviceName,`成功写入 237 每轮count本地Excel文件`);
            // writeLogExcel(deviceName,`LogInfo:成功写入237 每轮count 网络Excel文件`);
        } else {
            print(deviceName + "--写入本地Excel失败! 错误是: " + lastError());
        }
    } catch (err) {
